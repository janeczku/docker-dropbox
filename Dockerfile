...
RUN echo 'deb http://linux.dropbox.com/debian jessie main' > /etc/apt/sources.list.d/dropbox.list \
	...
	&& apt-get -qqy install ca-certificates dropbox \
...
# Dropbox has the nasty tendency to update itself without asking. While doing this it also fills the
# file system over time by writing rather large update files to /dbox and /tmp.
